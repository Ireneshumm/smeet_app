...
| **从上述页面栈内进入的子页** | 例如 **ChatRoom**、组局创建/详情、**Matches** 等，凡通过 `Navigator.push`（含 `MaterialPageRoute`）进入、**不依赖** MVP 命名路由的，均视为正式路径。 |
...
**若未来增加** 内测 flavor 或 `dart-define` 开关（例如 `ENABLE_MVP_ROUTES`），**可**在灰度包中恢复部分命名路由，使内测员能进入：

- Inbox / Feed / Create hub / Notifications / Profile MVP / Setup demo  

**产品/运营注意**：灰度包若打开上述路由，**商店描述与截图** 仍应按 **正式 release 边界**（§1、§5）撰写，避免用户误以为商店版具备相同入口。
...
| 能力 | 原因 |
|------|------|
| **应用内「通知中心」与真实推送** | Notifications MVP 多为 **Mock / 预览**，无完整后端通知产品化。 |
...
商店素材避免出现 **假通知列表、Mock 切换、MVP 调试菜单**，除非明确为 **TestFlight / 内测** 且与上架包不一致。
...
| **Notifications** | 无应用内入口；且页面为 Mock | 写 **「推送/通知中心即将推出」** 或完全不提；勿承诺已上线。 |
